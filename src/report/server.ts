...
function respond(res: http.ServerResponse, status: number, body: string, type = "text/html; charset=utf-8") {
...
function respond404(res: http.ServerResponse) {
  respond(res, 404, `<!doctype html><html><body style="font-family:sans-serif;padding:40px;text-align:center"><h1>404</h1><p>Not found</p><a href="/">Back to reports</a></body></html>`);
}
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (apiRunMatch) {
        ...
        if (!manifest) { respondJson(res, 404, { error: "Run not found" }); return; }
      ...
      if (runMatch) {
        ...
        if (!manifest) { respond404(res); return; }
      ...
      respond(res, 500, `<!doctype html><html><body style="font-family:sans-serif;padding:40px"><h1>500 Internal Error</h1><pre>${msg}</pre></body></html>`);
    }
  });
