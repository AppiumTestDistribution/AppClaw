...
const STEP_SNIPPETS = [
  ...
  { label: "type in", insert: "type '${1:text}' in ${2:field}", detail: "Type text into a specific field" },
  { label: "type secret", insert: "type '\\${secrets.${1:key}}' in ${2:field}", detail: "Type a secret value" },
  { label: "type variable", insert: "type '\\${variables.${1:key}}' in ${2:field}", detail: "Type a variable value" },
  ...
  { label: "wait until visible", insert: "wait until ${1:element} is visible", detail: "Wait for element to appear" },
  { label: "wait until gone", insert: "wait until ${1:element} is gone", detail: "Wait for element to disappear" },
  { label: "wait until screen loaded", insert: "wait until screen is loaded", detail: "Wait for screen to stabilize" },
  ...
  { label: "scroll until visible", insert: "scroll down until ${1:element} is visible", detail: "Scroll until element found" },

  // Assertions
  { label: "verify", insert: "verify ${1:text} is visible", detail: "Assert text is visible on screen" },
  { label: "assert", insert: "assert ${1:text} is visible", detail: "Assert text is visible on screen" },
  ...
  { label: "done with message", insert: "done: ${1:expected text on screen}", detail: "End flow and verify message" },
];

const TOP_LEVEL_KEYS = [
  { label: "name", insert: "name: ${1:flow_name}", detail: "Flow name" },
  { label: "description", insert: "description: ${1:What this test verifies}", detail: "Flow description" },
  ...
  { label: "setup", insert: "setup:\n  - ${1:open MyApp}", detail: "Setup phase — initialization steps" },
  { label: "steps", insert: "steps:\n  - ${1:tap Login}", detail: "Test phase — main test actions" },
  { label: "assertions", insert: "assertions:\n  - ${1:verify Dashboard is visible}", detail: "Assertion phase — verification checks" },
];
  ...
  provideCompletionItems(
    ...
    if (varMatch && workspaceRoot) {
      ...
      for (const key of keys) {
        ...
        item.documentation = scope === "secrets"
