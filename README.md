...
<details>
<summary><strong>Local Qwen vision (no cloud, no Gemini key)</strong></summary>

**Fully local vision** with **Qwen2.5-VL** via [LM Studio](https://lmstudio.ai) — both the _action decision_ (tap/type/swipe/assert) and the _element grounding_ run on the local model. Pixel-perfect, ~1.5–2s/step, no cloud calls for vision. (`LLM_PROVIDER` is still required for any non-vision reasoning, but vision mode itself never leaves your machine.)

**LM Studio setup:** Discover tab → search `Qwen2.5-VL-7B-Instruct`, pick a build with the vision (👁️) badge (on Apple Silicon prefer an **MLX** build, e.g. `mlx-community/Qwen2.5-VL-7B-Instruct-4bit`; GGUF also works) → load it → Local Server tab → Start Server. Then copy the exact model id LM Studio shows into `STARK_VISION_MODEL`.

```env
LLM_PROVIDER=gemini                        # provider for any non-vision steps
LLM_API_KEY=your-gemini-api-key
AGENT_MODE=vision
VISION_PROVIDER=qwen                        # native Qwen2.5-VL (action + grounding, all local)
STARK_VISION_BASE_URL=http://localhost:1234 # /v1 is appended automatically if omitted
STARK_VISION_MODEL=qwen2.5-vl-7b-instruct   # exact id from LM Studio
```

**Picking a local model — coordinate contract matters.** Local VLMs ground in one of two formats, and the provider must match:

| Model family                                      | `VISION_PROVIDER` | Why                                                                                                                             |
| ------------------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| **Qwen2.5-VL**                                    | `qwen`            | Returns native pixel `bbox_2d`; the qwen path scales it to device pixels.                                                       |
| **Gemma / Gemini-family** (e.g. `gemma-4-e4b-it`) | `stark`           | Speaks the Gemini normalized `[y,x]` 0–1000 contract that the stark path already uses. Keep `STARK_VISION_COORDINATE_ORDER=yx`. |

Both need ≥768px screenshots to ground small targets — when `STARK_VISION_BASE_URL` is set, AppClaw automatically sends `QWEN_VISION_MAX_EDGE_PX` (default 768) instead of the cloud 512px. Tested models: `qwen2.5-vl-7b-instruct` ✅ and `gemma-4-e4b-it` ✅ ground reliably; reasoning models (`qwen3.5-9b`) hallucinate coordinates and are not recommended.
...
| **Vision**                |                    |                                                                                                                                                                                                                                                                                                                                                                         |
| `VISION_PROVIDER`         | `stark`            | Grounding provider for `AGENT_MODE=vision`: `stark` (df-vision + Gemini) or `qwen` (native Qwen2.5-VL via local LM Studio)                                                                                                                                                                                                                                              |
| `STARK_VISION_BASE_URL`   | —                  | OpenAI-compatible local vision server (e.g. `http://localhost:1234/v1`). Required for `VISION_PROVIDER=qwen`; optional for a forced-format local `stark` server.                                                                                                                                                                                                        |
| `STARK_VISION_MODEL`      | (auto)             | Model id for the local vision server (e.g. `qwen2.5-vl-7b-instruct`). For `stark` cloud, falls back to `LLM_MODEL` (Gemini) or a built-in default.                                                                                                                                                                                                                      |
| `QWEN_VISION_MAX_EDGE_PX` | `1024`             | Max screenshot edge (px) sent to local vision models (`VISION_PROVIDER=qwen`, or any model behind a local `STARK_VISION_BASE_URL`). Local VLMs ground in pixel space so accuracy scales with resolution; latency is inference-bound and barely moves (768→1280 ≈ +250ms). Dense Play Store screen: 768 → 3/6, 1024 → 5/6, 1280 → 6/6. Raise to 1280 for very dense UIs. |
| **Agent**                 |                    |                                                                                                                                                                                                                                                                                                                                                                         |
